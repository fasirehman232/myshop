...
def admin():
    ...
    if request.method == "POST":
        if request.form.get("action") == "update_banner":
            ...
            if name and price:
                cur.execute("INSERT INTO products (name, price, image, category) VALUES (?, ?, ?, ?)", (name, price, image, category))
        conn.commit()
...
def edit(id):
    ...
    if request.method == "POST":
        ...
        if name and price:
            if image:
                cur.execute("UPDATE products SET name=?, price=?, image=?, category=? WHERE id=?", (name, price, image, category, id))
            else:
                cur.execute("UPDATE products SET name=?, price=?, category=? WHERE id=?", (name, price, category, id))
            conn.commit()
            conn.close()
            return redirect(url_for("admin"))
